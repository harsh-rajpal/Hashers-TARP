...
<font size="10">
<div align="justify">
&nbsp;&nbsp;&nbsp;&nbsp;Artificial intelligence is currently being used in many real-time applications, and there have been numerous developments in deep learning techniques implemented in the field of computer vision, which have greatly improved various industries such as video surveillance, industrial automation, self-driving vehicles, military, medical industry, and more. These developments have achieved excellent results, allowing objects to be localized, predicted, and classified based on the detected object. In daily traffic, traffic signals play a crucial role in ensuring the smooth flow of road traffic and increasing roadway traffic security. Traffic signals encompass not only signal lamps, signs, and markings, but also traffic police commands.
</div><br>

<div align="justify">
&nbsp;&nbsp;&nbsp;&nbsp;However, recognizing traffic police gestures using current methods is difficult and poses two challenges. Firstly, most existing methods extract pixel-level features from RGB images, which are not easily interpretable due to the lack of gesture skeleton features and may result in inaccurate recognition due to background noise. Therefore, appropriate and effective features representing traffic police gestures need to be chosen and extracted. However, traffic police typically work in complex and unpredictable environments, which can introduce interference and render features uninterpretable. To address this issue, a gesture skeleton extractor (GSE) can be used to extract and improve interpretable skeleton coordinate information. Compared to extracted pixel-level features, skeleton information can eliminate background interference and make features interpretable through coordinates and the proposed attention mechanism. Secondly, existing deep learning methods are not suitable for handling gesture skeleton features.
</div><br>

<div align="justify">
&nbsp;&nbsp;&nbsp;&nbsp;Previous studies faced some limitations, such as relying on handcrafted components or pixel-level information to extract skeleton features. These methods fail to establish the relationship between skeleton joint coordinates and gestures, lack interpretable topological features, demonstrate weak generalization capability, and exhibit poor recognition performance. For autonomous vehicles to comply with road traffic laws, they must also recognize hand signals from traffic controllers in real-time, which can be challenging given the high speed of the vehicles. Many skeleton-based methods require high computational load and are unsuitable for real-time processing. While using depth sensors can reduce computational load, it requires expensive devices. 
</div><br>
  
 <div align="justify">
&nbsp;&nbsp;&nbsp;&nbsp;Prior approaches have used skeleton-based action recognition to identify hand signals in input videos. These methods required preprocessing of video streams to extract skeletons, which added extra burden and reduced overall processing time. Additionally, previous works were only applied to videos taken indoors or with a limited number of backgrounds, making them unsuitable to generalize to real-world problems. To accurately detect hand signals, it is essential to distinguish between police officers giving appropriate hand signals and those not delivering meaningful signals. The accuracy of detection is significantly affected by the ability to differentiate situations in which signals are given from situations when no intentional signals are made. 
</div><br>

<div align="justify">
&nbsp;&nbsp;&nbsp;&nbsp;In summary, the paper discusses the challenges in recognizing hand signals from real-world environments without the need for preprocessing. Previous methods relied on skeleton-based action recognition, which required preprocessing of video streams to extract skeletons, resulting in extra burden and reduced processing time. Moreover, previous works were applied to indoor videos or videos with limited backgrounds, making it challenging to generalize the trained neural network to real-world problems. To overcome these challenges, the paper proposes a new method that utilizes an attention-based spatial-temporal graph convolutional network (ASTGCN) that can recognize hand signals from raw video streams without the need for preprocessing. The proposed method achieved higher accuracy than the previous methods and can distinguish between situations where signals are given and situations when no intentional signals are made. The results demonstrate the potential of using deep learning methods to recognize hand signals from real-world environments, which can be useful in various applications, such as traffic management, public safety, and military operations.
</div><br></font>
...
### Dependencies :
 ...
 ### 1. ediapipe Library(for Realtime Detection)
...
<table size="10">
  ...
  <tr>
    ...
    <td><ul><li>inрut video fоrm thе саmеrа intо саr аnd dеtесt оbjесt</li><li>SSD(Singlе Shоt MultiBох Dеtесtоr).</li><li>DNN(Dеер Nеurаl nеtwоrk)</li></ul></td>
    <td><ul><li>It саn оnlу dеtесt саr,реrsоn,аnimаl,trееs,Bus,dividеr,biсусlе еtс.</li></ul</td>
   </tr>
